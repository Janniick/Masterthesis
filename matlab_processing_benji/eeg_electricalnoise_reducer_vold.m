...
% year   : 2024
function EEG = eeg_electricalnoise_reducer_vold(EEG, omits, display_plot, plot_output_path, filename, where_notch, type, sequential_min, times_max_mad)
    arguments
        EEG
    ...
    % omit, glue later back together
    EEG_clean.data = EEG_clean.data(:,~omits);

    eeg_channel_indices = 1:size(EEG_clean.data, 1);

    if display_plot == 1
        fig = figure('Visible', 'on');
    elseif ~strcmp(plot_output_path, '')
        fig = figure('Visible', 'off');
    end

    for chaneli = eeg_channel_indices
        fprintf('%s', EEG_clean.chanlocs(chaneli).labels);
    
        % notch filter order and zero padding to warm up the filter (10 seconds)
        nf_order = 300;
        pad_length = 10*EEG_clean.srate;
    
        % notch filter
        for notch_run = [1, 0.5, 0.25, 0.1, 0.1] % how much +- Hz
            for notch_freq = where_notch(where_notch<EEG_clean.srate/2)
                notch_filt = designfilt('bandstopfir', ...
                    ...
                    'SampleRate', EEG_clean.srate);
                filtered_notch = filtfilt(notch_filt, [zeros(1, pad_length), double(EEG_clean.data(chaneli, :))]);
                EEG_clean.data(chaneli,:) = filtered_notch(pad_length + 1:end);
            end
        end
        clear filtered_notch;

        
        nrsequences = max(floor((size(EEG_clean.data,2)/EEG_clean.srate/60)/sequential_min),1);
        for seqi = 1:nrsequences
            % sequence from to index
            fromto = ((seqi-1)*sequential_min*60*EEG_clean.srate+1):seqi*sequential_min*60*EEG_clean.srate;

            % get info on FFT and do FFT
            duration_fft = length(EEG_clean.data(chaneli,fromto));

            y = fft(EEG_clean.data(chaneli,fromto));

            magnitude_logy = 10*log10(abs(y(1:floor(duration_fft/2))).^2);
            phase_y = angle(y);

            xw = EEG_clean.srate/duration_fft*(1:floor(duration_fft/2));

            % rolling median
            rolling_window = 2*duration_fft/EEG_clean.srate; % 2Hz steps
            ...
            freq_to_regulate = freq_to_regulate(freq_to_regulate > 0.2);
            freq_to_regulate = freq_to_regulate(freq_to_regulate < max(xw)-0.2);
            ...
            if strcmp(type, 'zero') | strcmp(type, 'median')
                EEG_clean.data(chaneli,fromto) = ifft(y,'symmetric');
            end
    ...
    % glue back together
    EEG.data(:,~omits) = EEG_clean.data;
    %EEG_clean.data(:,omits) = EEG.data(:,omits);
end