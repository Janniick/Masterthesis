...
for i = 1:numel(edf_files) %randperm(numel(edf_files)) % 1:numel(edf_files)
    ...
    num_preprocessing_steps = 3; % Adjust this as needed
    ...
    subplot(num_preprocessing_steps, num_channels, ith_channel + (num_preprocessing_steps_array(preprocessing_step) - 1)* num_channels);
            ...
            colormap('jet');
            colorbar;
            annotation('textbox', [0, row_positions(num_preprocessing_steps_array(preprocessing_step)), 1, 0.05], 'String', 'ASR', ...
            'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontSize', 12);
    end
    ...
    xlim([0 40]);
    ylabel('Power (dB)');
    titleplot = ['Comparison of EEG Power Spectra in ', condition]
    title(titleplot);
    % Add a legend to identify which line corresponds to which EEG data
    legend({'Raw data', 'Detrend', 'ASR'});
    ...
    hold on; 
    plot(freqs2, spectra2(1,:)-spectra5(1,:), 'cyan-*');
    % Adding labels and title
    xlabel('Frequency (Hz)');
    xlim([0 40]);
    ylabel('Power (dB)');
    titleplot = ['Difference of EEG Power Spectra in ', condition]
    title(titleplot);
    % Add a legend to identify which line corresponds to which EEG data
    legend({'Raw data - Detrend', 'Detrend - ASR'});
    % Enable grid for better visibility
    grid on;
    % Release the hold on the plot
    hold off;
