...
for i = 1:numel(edf_files) %randperm(numel(edf_files)) % 1:numel(edf_files)
    ...
    condition = 'N3GHBfullnightremoved';
...
difference_data = EEG_EOG.data - EEG_ECG.data;  % Simple difference between post and pre
...
% Update the chanlocs to reflect the new channel configuration
selectedEEG.chanlocs = [EEG_ECG.chanlocs(pre_idx) EEG_ECG.chanlocs(diff_idx)];

% Update the channel labels to indicate 'diff' for difference channels
for i = 1:length(diff_idx)
    selectedEEG.chanlocs(length(pre_idx) + i).labels = [selectedEEG.chanlocs(length(pre_idx) + i).labels '_diff'];
end

% Update the number of channels
selectedEEG.nbchan = numel(selectedEEG.chanlocs);
...
fig = figure('visible', 'on', 'WindowStyle', 'normal');
...
EEG_steps = {EEG_raw, EEG_raw, EEG_detrend_removed, EEG_noise, EEG_ECG, EEG_EOG};
...
%%%% same for nonREM
    ...
    % Starttime 
    startMinnonREM = 4 * 60 + 0;
    endMinnonREM = 4 * 60 + 30;
