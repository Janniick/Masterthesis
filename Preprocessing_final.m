...
for i = 1:numel(edf_files) %randperm(numel(edf_files)) % 1:numel(edf_files)
    ...
    results = cell(length(bad_channels), 3);  % Create a cell array with 3 columns
    % Loop through bad channels and save them with 'thename' and the corresponding value_chan
    for i = 1:length(bad_channels)
        results{i, 1} = thename;                         % First column: 'thename'
        results{i, 2} = bad_channels{i};                 % Second column: bad channel name
        results{i, 3} = value_chan(bad_channel_indices(i)); % Third column: corresponding value
    end
    % Append current results to the master results array
    all_bad_channels = [all_bad_channels; results];  % Append results for the current file
    ...
    % Prepare results as a cell array or table
    bad_data = cell(1, 5);
    bad_data{1,1} = thename;
    bad_data{1,2} = round(length(EEG_interp.times)/(EEG_interp.srate*60), 2);
    bad_data{1,3} = round(length(EEG_interp_removed.times)/(EEG_interp.srate*60),2);
    bad_data{1,4} = round(length(EEG_interp.times)/(EEG_interp.srate*60) - length(EEG_interp_removed.times)/(EEG_interp.srate*60),2);
    bad_data{1,5} = round((100/length(EEG_interp.times))*length(EEG_interp_removed.times),2);
    % Append current bad_data to the master all_bad_data array
    all_bad_data = [all_bad_data; bad_data];  % Append results for the current file
    ...
    fprintf('---ECG removal\n');
    EEG_ECG = EEG_reref;
    EEG_ECG = eeg_jelena_ecg_removal(EEG_reref, ECG, cut_chunks);
    ...
    save(filePath, 'EEG_ASR');
    % Optionally display the path where the file was saved
    disp(['Saved EEG_ASR in ', filePath]);

    %% clear most of the data
    clearvars -except folderpath i edf_files web_files study selected_channels windowDuration SDCutoff
end
%% Save bad channel information
% Convert the cell array to a table for better structure
results_table = cell2table(all_results, 'VariableNames', {'Filename', 'BadChannel', 'Mean Correlation'});
% Define the full path including the filename
csv_file_path = fullfile(folderpath, [study, '_bad_channels.csv']);
% Save the table as a CSV file in the specified folder
writetable(results_table, csv_file_path);
disp(['Bad channel info saved to: ', csv_file_path]);

%% Save bad data information
% Convert the cell array to a table for better structure
all_bad_data_table = cell2table(all_bad_data, 'VariableNames', {'Filename', 'Total recording duration in minutes', 'Usable recording duration in minutes', 'Bad data duration in minutes', 'Usable recording duration percentage'});
% Define the full path including the filename
csv_file_path = fullfile(folderpath, [study, '_bad_data.csv']);
% Save the table as a CSV file in the specified folder
writetable(all_bad_data_table, csv_file_path);
disp(['Bad data info saved to: ', csv_file_path]);
