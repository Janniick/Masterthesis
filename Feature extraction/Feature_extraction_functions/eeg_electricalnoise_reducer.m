...
% year   : 2024
function EEG = eeg_electricalnoise_reducer(EEG, omits, nrparallel,lowestfreq,highestfreq, display_plot, plot_output_path, filename, where_notch, type, sequential_min, times_max_mad)
    arguments
        EEG
        ...
        nrparallel (1,1) {mustBeNumeric} = max(1, floor(0.8 * feature('numcores')))
    ...
    nr_chans = length(eeg_channel_indices);
    ...
    if isempty(gcp('nocreate'))
        ...
        disp('A parallel pool is open. Closing it.');
        delete(gcp('nocreate'));
    end
    parpool(nrparallel);
    ...
    eegsliced = cell(nr_chans, 1);
    % Loop through each channel and assign the data to the cell array
    for chii = 1:nr_chans
        eegsliced{chii} = EEG.data(chii, :);
    end
    eegsize = size(EEG.data, 1);
    fprintf('----start\n');
    parfor chaneli = 1:eegsize
        fprintf('%s', eeglabels{chaneli});
    
        % notch filter order and zero padding to warm up the filter (10 seconds)
        nf_order = 300;
        pad_length = 10*eegsrate;
    
        % notch filter
        for notch_run = [1, 0.5, 0.25, 0.1, 0.1] % how much +- Hz
            for notch_freq = where_notch(where_notch<eegsrate/2)
                notch_filt = designfilt('bandstopfir', ...
                    ...
                    'SampleRate', eegsrate);
                filtered_notch = filtfilt(notch_filt, [zeros(1, pad_length), double(eegsliced{chaneli})]);
                eegsliced{chaneli} = filtered_notch(pad_length + 1:end);
            end
        end
        % clear filtered_notch;

        
        nrsequences = max(floor((length_keep/eegsrate/60)/sequential_min),1);
        %parfor
        for seqi = 1:nrsequences
            % sequence from to index
            fromto = ((seqi-1)*sequential_min*60*eegsrate+1):seqi*sequential_min*60*eegsrate;

            % get info on FFT and do FFT
            duration_fft = length(eegsliced{chaneli}(keep_idx(fromto)));

            y = fft(eegsliced{chaneli}(keep_idx(fromto)));

            magnitude_logy = 10*log10(abs(y(1:floor(duration_fft/2))).^2);
            phase_y = angle(y);

            xw = eegsrate/duration_fft*(1:floor(duration_fft/2));

            % rolling median
            rolling_window = 2*duration_fft/eegsrate; % 2Hz steps
            ...
            if length(freq_to_regulate) > 1
                ...
                if lgroups > 0
                    ...
                    for ig = 1:lgroups
                        ...
                        [~, idxm] = max(magnitude_logy(fmin_idx_l:fmin_idx_r));
            ...
            if strcmp(type, 'zero') | strcmp(type, 'median')
                eegsliced{chaneli}(keep_idx(fromto)) = ifft(y,'symmetric');
            end
        ...
        if display_plot == 1 | ~strcmp(plot_output_path, '')
            ...
            title(sprintf('Channel: %s', eeglabels{chaneli}));
            hold off;
        end
