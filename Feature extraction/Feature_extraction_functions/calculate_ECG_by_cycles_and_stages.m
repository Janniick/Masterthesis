...
for cycle_idx = 1:nr_cycles
                ...
                for idx = epoch_indices
                    if idx <= nr_epochs
                        ...
                        try
                            ...
                            fprintf('Error in calculate_ECG_values for cycle %d, epoch %d: %s\n', cycle_idx, idx, ME.message);
                        ...
                        % Convert ECG_values struct to an array
                        ECG_values_array = struct2array(ECG_values);
                        % Replace NaN values with a large placeholder for concatenation (e.g., 1e12)
                        ECG_values_array(isnan(ECG_values_array)) = 1e12;
                        % Concatenate ECG values into NREM_data
                        NREM_data = [NREM_data; ECG_values_array];
            ...
            elseif stage == 5  % REM stage
                ...
                for idx = epoch_indices
                    if idx <= nr_epochs
                        ...
                        ECG_values_array(isnan(ECG_values_array)) = 1e12;
                        % Concatenate ECG values into NREM_data
                        REM_data = [REM_data; ECG_values_array];
                    end
                end
            end
        end
    ...
    NREM_data(NREM_data == 1e12) = NaN;
    REM_data(REM_data == 1e12) = NaN;
